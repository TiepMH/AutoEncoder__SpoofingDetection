def cast_DOA_to_string(DOA):
    ...
    if len(temp) > 1:
        if DOA >= 0:
            ...
            kq = 'm' + str(abs(int(temp[0]))) + 'p' + temp[1]
    else:
        if DOA >= 0:
            kq = temp[0]
        else:
            kq = 'm' + str(abs(int(temp[0])))
    return kq + 'd'  # d stands for degrees
...
def srt_for_multiple_Bobs(n_Tx, snrdB_Bob, DOA_Bob_list):
    ...
    kq = '_B' + str(snrdB_Bob) + 'dB_' + Bob_angles
    return kq
...
def my_folder_name(n_Tx, n_Rx,
    ...
    name = 'nR' + str(n_Rx) \
            ...
            + 'E' + str(snrdB_Eve) + 'dB_' + DOA_Eve_str \
            + '_K' + str(K)
    return name
